...
data$pais[data$pais %in% names(table(data$pais))[table(data$pais)<=40]] <- "Other" 
...
t_atipicos = cbind("Conteo Atipicos" = apply(na.omit(data_num), 2,atipicos,15),"% Atipicos" = apply(na.omit(data_num), 2,atipicos,15)/1781*100)

t_atipicos[order(t_atipicos[,1]),]

# t_atipicos2 = cbind("Conteo Atipicos" = apply(na.omit(data_num), 2,atipicos2,15),"% Atipicos" = apply(na.omit(data_num), 2,atipicos2,15)/1781*100)

#t_atipicos2[order(t_atipicos2[,1]),]
...
```{r}
atip = table(unlist(sapply(data_num, atipicos2,5,1)))
...
data_model = na.omit(data[, -c(1,4,5,8,20)])
...
data_model = na.omit(data[, -c(1,8,20)])
...
id= which(sapply(data_model,class)!="factor")
