...
```{r plot num}
ggplot(data, aes(numeroips))+ geom_density(aes(fill=tipo),alpha=0.4)+labs(fill="Tipo")

ggplot(data, aes(largourl))+ geom_density(aes(fill=tipo),alpha=0.4) +labs(fill="Tipo")

ggplot(data, aes(numpaquetesdns))+ geom_density(aes(fill=tipo),alpha=0.4)+labs(fill="Tipo") 
...
```{r plot cat}
ggplot(data, aes(codificacion))+ geom_bar(aes(fill=tipo))+labs(fill="Tipo")
ggplot(data, aes(os))+ geom_bar(aes(fill=tipo))+labs(fill="Tipo")
ggplot(data, aes(pais))+ geom_bar(aes(fill=tipo))+labs(fill="Tipo")
...
Para el ajusto del modelo de Naive Bayes se utilizaron diferentes valores para el parámetro de laplace 
...
```
El modelo que mejor ajuste presentó fue aquel con laplace = 10 usando la corrección de kernel y utilizando el ajuste.
...
ggplot(df, aes(depth)) + 
 ...
 modelopoda <- train(tipo~., data = webTrain, method = "rpart",
  ...
  m <- modelopoda$finalModel 
 #rpart.plot(m) 
 df[which.max(df$kappa),]
  predicciones <- predict(modelo, newdata=webTrain) #type "raw" por defecto
  cm <- confusionMatrix(predicciones,webTrain$tipo, mode = "everything")
...
tunegrid <- expand.grid(.mtry=c(sqrt(17)))
modellist <- list()
...
fit1 <- train(tipo~., data=webTrain, method="rf", metric=metric, tuneGrid=tunegrid, trControl=control, ntree=500)
...
predictions_test<-predict(object=fit1, webTest)
 ...
 print("Training")
predictions<-predict(object=fit1, webTrain)
 confusionMatrix(predictions, factor(webTrain$tipo))$overall[1:2]
